...
- A transcript segment receives a label only when **exactly one voice** intersects it and the union of that voice's intervals covers at least **80%** of the segment. Overlap, speaker changes within an ASR segment, weak coverage, and zero-duration segments remain unknown. This threshold is a conservative alignment rule, not a confidence score.
...
- Rename a voice in the Transcript tab after checking the audio. Linked self-assignments update as user edits; explicit assignments to other named people remain independent. JSON, CSV, and ICS preserve the displayed owner; CSV also exports the speaker ID.
